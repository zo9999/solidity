...
void InstantiationRegistration::endVisit(TypeClassName const& _typeClassName)
{
	annotation(_typeClassName).typeClass = std::visit(util::GenericVisitor{
		[&](ASTPointer<IdentifierPath> _identifierPath) -> std::optional<TypeClass> {
			...
			{
				m_errorReporter.typeError(9817_error, _typeClassName.location(), "Expected type class.");
				return std::nullopt;
			}
		...
		[&](Token _token) -> std::optional<TypeClass> {
			...
			if (!builtinClass.has_value() || classRegistrationAnnotation.builtinClasses.count(*builtinClass) == 0)
				m_errorReporter.typeError(2658_error, _typeClassName.location(), "Invalid type class name.");
